...
func runCheck(cfg config.Config, checkers map[string]*checker.Checker, globalNotifiers []notifier.Notifier, stores *storeSet) {
	for _, m := range cfg.Monitors {
		executeAndPrint(checkers[m.Name], m, false, resolveNotifiers(m, globalNotifiers, cfg), stores)
	}
}
